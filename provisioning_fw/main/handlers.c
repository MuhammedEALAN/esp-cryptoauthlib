...
esp_err_t init_atecc608a(char *device_type, uint8_t i2c_sda_pin, uint8_t i2c_scl_pin, int *err_ret)
{
    ...
    // Set the I2C address of the ATECC608A to 0xC0
    cfg_ateccx08a_i2c_default.atcai2c.address = 0xC0;
    ESP_LOGI(TAG, "  . Initialize the ATECc With adress 0x%02x ...", cfg_ateccx08a_i2c_default.atcai2c.address);
    ESP_LOGI(TAG, "  . Set I2c baudrate to %ld ...", cfg_ateccx08a_i2c_default.atcai2c.baud);
...
esp_err_t get_cert_def(unsigned char *cert_def_array, size_t data_len, cert_type_t cert_type)
{
    ...
    {
        g_cert_def_common = g_cert_def_3_device;
    }
